            ...
            <div class="form-group">
                            ...
                            <select type="text" name="class_id" id="class_id" class="form-control"   placeholder="Class" required="">
                                <option value="">--Select Class--</option>
                                    ...
                                    while ($row = mysqli_fetch_array($result)) {
                                     ...
                                     ?>
                                            <option value="<?php echo $row["cl_id"];?>" data-capacity="<?php echo $qty; ?>">
                        ...
                        <div class="col-sm-9">
                            <select type="text" name="room_type_id" id="room_type_id" class="form-control"   placeholder="Room" required="">
                                <option value="">--Select Room Type--</option>
                                    <?php  
                                    $c1 = "SELECT id as tid, roomname FROM `room_type`";
                                    $result = $conn->query($c1);
                                    while ($row = mysqli_fetch_array($result)) {
                                        $s1 = "SELECT SUM(strenght) as qty FROM `room` WHERE type_id='".$row["tid"]."'";
                                        $sr = $conn->query($s1);
                                        if ($sr->num_rows > 0) {
                                            $sres = mysqli_fetch_array($sr);
                                            $qty=$sres['qty'];
                                     ...
                                     ?>
                                            <option value="<?php echo $row["tid"];?>" data-capacity="<?php echo $qty; ?>">
				...
				<div class="form-group">
                    <div class="row">
                        ...
                        <div class="col-sm-9">
                            <select type="text" name="room_id" id="room_id" class="form-control"   placeholder="Room" required="">
                                <option value="">--Select Room--</option>
                                    <?php  
                                    $c1 = "SELECT id as rid, name, type_id FROM `room`";
                                    $result = $conn->query($c1);
                                    while ($row = mysqli_fetch_array($result)) {
                                        $s1 = "SELECT SUM(strenght) as qty FROM `room` WHERE type_id='".$row["tid"]."'";
                                        $sr = $conn->query($s1);
                                     ...
                                     ?>
                                            <option value="<?php echo $row["rid"];?>" data-capacity="<?php echo $qty; ?>">
                                                <?php echo $row['name'];?>
                                            </option>
                                            <?php
                                        }
				...
				<div class="form-group">
                    <div class="row">
                        ...
                        <div class="col-sm-9">
                            <select type="text" name="teacher_id" id="teacher_id" class="form-control"   placeholder="Staff" required="">
                                <option value="">--Select Staff--</option>
                                    <?php  
                                    $c1 = "SELECT id as tid, tfname from tbl_teacher";
                                    $result = $conn->query($c1);
                                        while ($row = mysqli_fetch_array($result)) {?>
